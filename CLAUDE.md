...
- Service worker: cache-first for shell + cross-origin runtime. CDN bundles precached on install (see Gotcha #9). Does NOT intercept `data/*.json` — that lives in IndexedDB instead (Gotcha #10).
- Offline-saved poems live in **IndexedDB** (`ganjoor` database, `data` store), keyed by the `data/`-relative path. The data layer's `fetchJson()` checks IDB first, then falls back to the network, so saved poems load instantly with no SW round-trip. OfflineCache writes downloads directly into IDB.
- **When changing `index.html` substantively, bump the version constants in `sw.js`** (currently `SHELL_CACHE`/`RUNTIME` at `v17`). No `DATA_CACHE` to worry about any more.
...
9. **Opaque responses don't satisfy `resp.ok`** — and that broke offline mode for months. The PWA loads React/ReactDOM/Tailwind via `<script src="https://...">` tags with no `crossorigin` attribute, so the browser fetches them in **no-cors** mode and the SW sees an opaque response (`status: 0`, `ok: false`, `type: 'opaque'`). The original `cacheFirst` had `if (resp && resp.ok) cache.put(...)`, which silently skipped every cross-origin script. The runtime cache stayed empty no matter how many times the user revisited online, so opening the PWA from the home screen in airplane mode showed a blank white page — `index.html` loaded fine but React never executed. Two-part fix in `sw.js`: (a) precache the CDN URLs in `install` using `fetch(u, {mode: 'no-cors'})` + `cache.put` (note: `cache.add` rejects opaque responses, so use `put` manually); (b) extend the runtime `cacheFirst` gate to `(resp.ok || resp.type === 'opaque')` so fonts and any other no-cors fetches also get cached opportunistically.

10. **Cache Storage doesn't scale to thousands of entries on iOS Safari.** The original OfflineCache stored all ~17K poem JSONs in a single Cache Storage namespace (`ganjoor-data-v13`). Correctness was fine, but iOS Safari indexes the namespace on PWA process startup, which made cold start 8+ s — measured as `responseStart` at 8235 ms while individual fetches were all sub-15 ms (i.e. the bottleneck is before the document even begins loading). Verified by `caches.delete('ganjoor-data-v13')` → cold start dropped to 78 ms. Fixed by moving offline storage to **IndexedDB** in `index.html`: `fetchJson()` checks IDB first, the OfflineCache component writes downloads directly there, and the SW no longer intercepts `data/*.json` at all. Legacy `ganjoor-data-v*` caches are deleted on activate so existing users automatically reclaim ~300 MB and the slow cold-start. If you ever add a new feature that wants to cache many small entries, **do NOT reach for Cache Storage** — go straight to IndexedDB or you will reintroduce this regression.
